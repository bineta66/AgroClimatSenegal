...
watch(() => props.warehouse, populate, { immediate: true });
  ...
  <div v-if="show" class="fixed inset-0 bg-slate-900/45 grid place-items-center z-50 p-5" @click.self="handleClose">
    <div class="bg-white rounded-2xl w-full max-w-[480px] shadow-lg flex flex-col">
      <div class="flex items-start justify-between px-5 pt-5">
        ...
        <button class="border-0 bg-transparent cursor-pointer p-2 rounded-lg flex items-center justify-center transition hover:bg-slate-100 text-slate-500" title="Fermer" @click="handleClose">
      ...
      <div class="flex items-center justify-end gap-2 px-5 py-4">
        <button class="px-3.5 py-2.5 rounded-lg font-semibold text-sm border border-slate-200 bg-white text-slate-700 transition hover:bg-slate-50" @click="handleClose" :disabled="loading">Annuler</button>
